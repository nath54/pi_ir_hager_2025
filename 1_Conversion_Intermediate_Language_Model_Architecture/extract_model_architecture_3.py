                    ...
                    for target in item.targets:
                        if isinstance(target, ast.Name):
                            ...
                            if isinstance(item.value, ast.Call):
                                layer_type = item.value.func.attr if isinstance(item.value.func, ast.Attribute) else item.value.func.id
                                layer_args = {arg.arg: ast.literal_eval(arg.annotation) for arg in item.value.args}
                                self.model_blocks[self.current_model_visit].block_layers[layer_name] = lc.Layer(layer_name=layer_type, layer_parameters_kwargs=layer_args)
                ...
                if isinstance(item, ast.Assign):
                    for target in item.targets:
                        if isinstance(target, ast.Name):
                            ...
                            if isinstance(item.value, ast.Call):
                                func_name = item.value.func.attr if isinstance(item.value.func, ast.Attribute) else item.value.func.id
                                func_args = {arg.arg: ast.literal_eval(arg.annotation) for arg in item.value.args}
                                self.model_blocks[self.current_model_visit].forward_flow_control.append(lc.FlowControlFunctionCall(output_variables=[output_var], function_called=func_name, function_arguments=func_args))

                elif isinstance(item, ast.Expr) and isinstance(item.value, ast.Call):
                    func_name = item.value.func.attr if isinstance(item.value.func, ast.Attribute) else item.value.func.id
                    func_args = {arg.arg: ast.literal_eval(arg.annotation) for arg in item.value.args}
                    self.model_blocks[self.current_model_visit].forward_flow_control.append(lc.FlowControlFunctionCall(output_variables=[], function_called=func_name, function_arguments=func_args))
