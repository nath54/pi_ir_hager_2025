...
class Model(nn.Module) :
    def __init__(self) :
        ...
        self.ln_f = LayerNorm(n_embd) #final layer
    ...
    def forward (self,x) :
        ...
        # output_ConvEncode = [ module_c(xi) for ( xi , module_c) in zip  (torch.unbind(x, dim=1) , self.ConvEncode ) ]

        # # Process each time step through its corresponding ConvEncode module
        # # Use indexing instead of split to avoid as_strided
        # output_ConvEncode = []
        # for t in range(min(WT, 6)):
        #     # Extract single time step using indexing
        #     xi = x[:, t:t+1, :, :]  # Shape: [B, 1, FD1, FD2]
        #     # Process through corresponding module
        #     encoded = self.ConvEncode[t](xi)
        #     output_ConvEncode.append(encoded)
