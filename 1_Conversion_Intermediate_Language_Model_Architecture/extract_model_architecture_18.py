...
class ModelAnalyzer(ast.NodeVisitor):
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        ...
        if len(node.targets) != 1:
            # Skip multi-target assignments for simplicity
            return

        target = node.targets[0]
        value = extract_expression(node.value)

        # Case 1: Global constant (outside any class or function)
        if not self.current_model_visit and not self.current_function_visit and isinstance(target, ast.Name) and value:
            type_str = "int" if isinstance(value, lc.ExpressionConstantNumeric) else "str" if isinstance(value, lc.ExpressionConstantString) else "list"
            self.global_constants[target.id] = (type_str, value)
            return

        # Case 2: Assignment inside a function
        if self.current_model_visit and self.current_function_visit:
            ...
            if self.current_function_visit in current_block.block_functions:
                ...
                if isinstance(target, ast.Name) and value:
                    # Add as a variable assignment in the function's flow control
    ...
    def visit_AugAssign(self, node: ast.AugAssign) -> None:
        """
        Handles augmented assignment statements (e.g., `x += y`).
        ...
        target = node.target
        value = extract_expression(node.value)
        op = {ast.Add: "+", ast.Sub: "-", ast.Mult: "*", ast.Div: "/"}.get(type(node.op), None)

        if not op or not isinstance(target, ast.Name) or not value:
            return

        # Only handle inside functions for now
        if self.current_model_visit and self.current_function_visit:
            current_block = self.model_blocks[self.current_model_visit[-1]]
            if self.current_function_visit in current_block.block_functions:
                func = current_block.block_functions[self.current_function_visit]
                # Convert augmented assignment to a binary operation
                result_var = target.id
                func.function_flow_control.append(
                    lc.FlowControlBasicBinaryOperation(
                        output_var_name=result_var,
                        input1_var_name=result_var,
                        operation=op,
                        input2_var_name=value.var_name if isinstance(value, lc.ExpressionVariable) else f"temp_{id(value)}"
                    )
                )
                # If the value is a constant, initialize it first
                if not isinstance(value, lc.ExpressionVariable):
                    func.function_flow_control.insert(
                        -1,  # Before the binary operation
                        lc.FlowControlVariableInit(
                            var_name=f"temp_{id(value)}",
                            var_type="Any",
                            var_value=value
                        )
                    )
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign) -> None:
        ...
        #
        target = node.target
        value = extract_expression(node.value) if node.value else None
        ...
        # Case 1: Global constant with annotation
        ...
        if self.current_model_visit and self.current_function_visit and isinstance(target, ast.Name):
            ...
            if self.current_function_visit in current_block.block_functions:
                func = current_block.block_functions[self.current_function_visit]
                # Use FlowControlVariableInit if there's a value, otherwise just declare
                if value:
                    func.function_flow_control.append(
                        lc.FlowControlVariableInit(
                            var_name=target.id,
    ...
    def visit_AssignStmt(self, node: ast.AST) -> None:
        """
        Placeholder for AssignStmt (not a standard AST node, likely a typo or custom node).

        Args:
            node (ast.AST): The node to visit.
        """
        # Note: There’s no `AssignStmt` in Python’s standard `ast` module, assuming this is a mistake or custom extension
        # For now, we’ll treat it as a no-op unless clarified otherwise
        ast.NodeVisitor.generic_visit(self, node)
