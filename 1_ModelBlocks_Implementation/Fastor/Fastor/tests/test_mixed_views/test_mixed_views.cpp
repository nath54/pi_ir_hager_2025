...
void run_mixed_views() {
    ...
    // Row-wise operations
    {
        ...
        FASTOR_DOES_CHECK_PASS(abs(b1.sum() - 12) < Tol);
        Tensor<T,3> b2 = a(1,all);
        FASTOR_DOES_CHECK_PASS(abs(b2.sum() - 12) < Tol);
        Tensor<T,1,1,3> b3 = a(1,all);
        FASTOR_DOES_CHECK_PASS(abs(b3.sum() - 12) < Tol);
        Tensor<T,1,1,3,1> b4 = a(1,all);
        FASTOR_DOES_CHECK_PASS(abs(b4.sum() - 12) < Tol);
        Tensor<T,1,1,3,1,1> b5 = a(1,all);
        FASTOR_DOES_CHECK_PASS(abs(b5.sum() - 12) < Tol);
        // Tensor<T,3,1> b6 = a(1,all);   // this is disallowed
        // FASTOR_DOES_CHECK_PASS(abs(b6.sum() - 12) < Tol);
    }
...

    {
        ...
        FASTOR_DOES_CHECK_PASS(abs(b1.sum() - 22) < Tol);
        Tensor<T,4> b2 = a(1,all);
        FASTOR_DOES_CHECK_PASS(abs(b2.sum() - 22) < Tol);
        Tensor<T,1,1,4> b3 = a(1,all);
        FASTOR_DOES_CHECK_PASS(abs(b3.sum() - 22) < Tol);
        Tensor<T,1,1,4,1> b4 = a(1,all);
        FASTOR_DOES_CHECK_PASS(abs(b4.sum() - 22) < Tol);
        Tensor<T,1,1,4,1,1> b5 = a(1,all);
        FASTOR_DOES_CHECK_PASS(abs(b5.sum() - 22) < Tol);
        // Tensor<T,4,1> b6 = a(1,all); // this is disallowed
        // FASTOR_DOES_CHECK_PASS(abs(b6.sum() - 22) < Tol);
    }
    ...
    // Column-wise operations
    {
        ...
        FASTOR_DOES_CHECK_PASS(abs(b1.sum() - 5) < Tol);
        Tensor<T,2> b2 = a(all,1);
        FASTOR_DOES_CHECK_PASS(abs(b2.sum() - 5) < Tol);
        Tensor<T,1,1,2> b3 = a(all,1);
        FASTOR_DOES_CHECK_PASS(abs(b3.sum() - 5) < Tol);
        Tensor<T,1,1,2,1> b4 = a(all,1);
        FASTOR_DOES_CHECK_PASS(abs(b4.sum() - 5) < Tol);
        Tensor<T,1,1,2,1,1> b5 = a(all,1);
        FASTOR_DOES_CHECK_PASS(abs(b5.sum() - 5) < Tol);
        // Tensor<T,1,2> b6 = a(all,1);   // this is disallowed
        // FASTOR_DOES_CHECK_PASS(abs(b6.sum() - 5) < Tol);
    }
    ...
    // Assigning 2D view to nD view
    {
        ...
        b(all,0,1) = a(all,1);
        FASTOR_DOES_CHECK_PASS(abs(b.sum() - 22) < Tol);
        b.zeros();
        b(0,all,1) = a(all,1);
        FASTOR_DOES_CHECK_PASS(abs(b.sum() - 22) < Tol);
        b.zeros();
        b(0,0,all) = a(0,all);
        FASTOR_DOES_CHECK_PASS(abs(b.sum() - 3) < Tol);
    }
    ...
    // Assigning nD view to 2D view
    {
        ...
        FASTOR_DOES_CHECK_PASS(abs(b.sum() - 232) < Tol);
        b.zeros();
        b(all,seq(0,2)) = a(all,seq(0,2),1);
        FASTOR_DOES_CHECK_PASS(abs(b.sum() - 164) < Tol);
        b.zeros();
        b(0,all) = a(1,1,all);
        FASTOR_DOES_CHECK_PASS(abs(b.sum() - 48) < Tol);
    }
    ...
    // Filter views
    {
        ...
        a(mask) = 100;
        FASTOR_DOES_CHECK_PASS(abs(a.sum() - 409) < Tol);
        Tensor<T,7> b = a(mask) + 1;
        FASTOR_DOES_CHECK_PASS(abs(b.sum() - 407) < Tol);

        Tensor<T,2,2> aa; aa.fill(2);
        Tensor<bool,2,2> baa = {{false,true},{true,false}};
        aa(baa) += 1 + aa;
        FASTOR_DOES_CHECK_PASS(abs(aa.sum() - 14) < Tol);
        aa(baa) -= 1 + aa;
        aa(baa) = 0;
        FASTOR_DOES_CHECK_PASS(abs(aa.sum() - 4) < Tol);
        aa(baa) *= 1 + aa;
        FASTOR_DOES_CHECK_PASS(abs(aa.sum() - 4) < Tol);
        aa(baa) /= 1 + aa;
        FASTOR_DOES_CHECK_PASS(abs(aa.sum() - 4) < Tol);
    ...
    // diag views
    {
        ...
        Tensor<T,4> b = diag(A);
        FASTOR_DOES_CHECK_PASS(abs(b.sum() - 30) < Tol);

        diag(A) = diag(A);
        FASTOR_DOES_CHECK_PASS(abs(A.sum() - 120) < Tol);

        diag(A) = 2*b;
        FASTOR_DOES_CHECK_PASS(abs(A.sum() - 150) < Tol);

        A.arange();
        diag(A) += 2*b;
        FASTOR_DOES_CHECK_PASS(abs(A.sum() - 180) < Tol);

        A.arange();
        diag(A) -= 2*b;
        FASTOR_DOES_CHECK_PASS(abs(A.sum() - 60) < Tol);

        A.arange();
        diag(A) *= 2*b;
        FASTOR_DOES_CHECK_PASS(abs(A.sum() - 790) < Tol);

        A.arange();
        b(0) = 1;
        diag(A) /= b;
        FASTOR_DOES_CHECK_PASS(abs(A.sum() - 93) < Tol);

        diag(A) = 2;
        FASTOR_DOES_CHECK_PASS(abs(sum(diag(A)) - 8) < Tol);

        diag(A) += 2;
        FASTOR_DOES_CHECK_PASS(abs(sum(diag(A)) - 16) < Tol);
        ...
        diag(A) = A % b - A % b;
        FASTOR_DOES_CHECK_PASS(abs(sum(diag(A)) - 0) < Tol);
    }
